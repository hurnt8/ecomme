...
<body style="font-family: Arial, sans-serif; color:#222; max-width:600px; margin:0 auto;">
    <h1 style="font-size:18px;">Nouvelle commande {{ $order->order_number }}</h1>
    <p>{{ $order->customer_name }} ({{ $order->customer_email }}) vient de passer une commande de {{ number_format((float) $order->total, 2) }}&nbsp;€.</p>
    ...
    <p style="margin-top:15px;">Adresse de livraison : <br><span style="white-space:pre-line;">{{ $order->shipping_address }}</span> ({{ $order->country }})</p>
</body>
</html>
