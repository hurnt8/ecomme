...
it('puts cold-season ranges first on the default ordering', function () {
    ...
    Product::factory()->create(['category_id' => $wood->id, 'name' => 'Scheite 50 cm', 'is_active' => true]);
    Product::factory()->create(['category_id' => $pools->id, 'name' => 'Frame-Pool rund', 'is_active' => true]);

    $html = $this->get(route('catalog'))->assertOk()->getContent();

    expect(strpos($html, 'Scheite 50 cm'))->toBeLessThan(strpos($html, 'Frame-Pool rund'));
});
...
it('leaves an explicit sort alone', function () {
    ...
    Product::factory()->create(['category_id' => $wood->id, 'name' => 'Scheite teuer', 'price' => 900, 'is_active' => true]);
    Product::factory()->create(['category_id' => $pools->id, 'name' => 'Pool billig', 'price' => 10, 'is_active' => true]);
    ...
    expect(strpos($html, 'Pool billig'))->toBeLessThan(strpos($html, 'Scheite teuer'));
});
